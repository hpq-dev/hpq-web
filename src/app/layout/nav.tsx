...
const Nav = () => {
    ...
    return <>
        <div className="fixed top-16 right-40 flex gap-4 item-center z-[1]">
            <motion.div
...
const Audio = ({ status = false, count = 6, onClick }: AudioProps) => {
    ...
    return <motion.div
        className='fixed top-16 z-[1] right-12 w-20 h-14 flex gap-1.5 items-center cursor-pointer click hover mix-blend-difference'
        onClick={onClick}
        whileTap={{
            scale: .8,
            transition: { duration: .2 }
